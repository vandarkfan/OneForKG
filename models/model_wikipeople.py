...
from helper import get_performance_wikipeople
import json
...
class T5Finetuner(pl.LightningModule):
    ...
    def training_step_dan(self, batched_data, batch_idx):
        ...
        if self.configs.use_soft_prompt:
            ...
            output = self.T5ForConditionalGeneration(input_ids=src_ids, attention_mask=src_mask, labels=labels,
                                                     entity_hidden_state=entity_hidden_state,addsource =addsource)
        loss = torch.mean(output.loss)
    ...
    def validation_step(self, batched_data, batch_idx):
        ...
        # src_ids, src_mask: .shape: (batch_size, padded_seq_len) .type: torch.tensor
            ...
            generated_text, scores_t5 = self.decode(src_ids, src_mask, batched_data, entity_hidden_state, addsource)
    ...
    def decode(self, src_ids, src_mask, batched_data,entity_hidden_state, addsource):
        ...
        def _next_candidate(configs, batch_idx, input_ids):
            real_triple = self.test_triple[batch_idx][:-2]
            index = tuple()
            ...
            pred_ids = tuple(ent_token_ids_in_trie[self.test_triple[batch_idx][-1]])
